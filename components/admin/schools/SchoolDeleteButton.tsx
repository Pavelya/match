/**
 * School Delete Button Component
 *
 * Client component for deleting schools with confirmation dialog.
 * Prevents deletion if school has coordinators or students.
 */
...
interface SchoolDeleteButtonProps {
  schoolId: string
  schoolName: string
...
export function SchoolDeleteButton({
  ...
  hasCoordinators,
  hasStudents
}: SchoolDeleteButtonProps) {
  const router = useRouter()
  const [showConfirm, setShowConfirm] = useState(false)
  const [deleting, setDeleting] = useState(false)
  const [error, setError] = useState<string | null>(null)

  const canDelete = !hasCoordinators && !hasStudents
  ...
  return (
    <>
      <button
        ...
        title={canDelete ? 'Delete' : 'Cannot delete: school has coordinators or students'}
      >
