...
export function StatCardSkeleton({ variant = 'default', className }: StatCardSkeletonProps) {
  if (variant === 'horizontal') {
    return (
      <div className={cn('flex items-center gap-3 rounded-xl border bg-card p-4', className)}>
        {/* Icon skeleton */}
        <div className="h-10 w-10 rounded-lg skeleton-bg animate-pulse" />
        <div className="flex-1 space-y-2">
          {/* Title skeleton */}
          <div className="h-3 w-16 rounded skeleton-bg animate-pulse" />
          {/* Value skeleton */}
          <div className="h-5 w-10 rounded skeleton-bg animate-pulse" />
        </div>
      </div>
    )
  }
  ...
  return (
    <div className={cn('rounded-xl border bg-card p-5', className)}>
      ...
      {/* Value skeleton */}
      <div className="h-8 w-16 rounded skeleton-bg animate-pulse" />
    </div>
  )
}
